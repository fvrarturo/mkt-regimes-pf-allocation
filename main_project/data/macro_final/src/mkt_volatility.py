...
Extracts the VIX (CBOE Volatility Index) as the market volatility indicator.

Methodology:
- Use VIX (Chicago Board Options Exchange Volatility Index)
- High-frequency (daily), converted to monthly (first value per month)
- VIX captures market implied volatility from S&P 500 options
"""
...
def create_market_volatility_factor(data_dir: Path, start_date: str = '1989-01-01', end_date: str = '2025-12-31') -> pd.DataFrame:
    """
    Create market volatility factor using VIX.
    ...
    # Use VIX value as the market volatility factor
