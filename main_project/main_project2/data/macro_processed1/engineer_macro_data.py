...
class MacroDataEngineer:
    ...
    def normalize_zscore(self, df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """
        Apply Z-score normalization (standardization) using expanding window to avoid look-ahead bias.
        
        For each time point, uses only historical data (up to that point) to calculate mean and std.
        This prevents look-ahead bias in time series analysis.
        
        Parameters:
        -----------
        df : pd.DataFrame
            DataFrame with columns to normalize (must have datetime index)
        columns : List[str]
        ...
        for col in columns:
            if col in df.columns:
                # Use expanding window: for each point, use only data up to that point
                # This prevents look-ahead bias
                expanding_mean = df[col].expanding(min_periods=2).mean()
                expanding_std = df[col].expanding(min_periods=2).std()
                
                # Normalize using expanding window statistics
                normalized = (df[col] - expanding_mean) / expanding_std
                
                # Handle cases where std is 0 (constant values up to that point)
                # Set to 0 (mean-centered) when std is 0
                normalized = normalized.fillna(0)
                
                df[f'zscore_{col}'] = normalized
        
        return df
    
    def normalize_minmax(self, df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """
        Apply Min-Max normalization (scaling to [0, 1]) using expanding window to avoid look-ahead bias.
        
        For each time point, uses only historical data (up to that point) to calculate min and max.
        This prevents look-ahead bias in time series analysis.
        ...
        for col in columns:
            if col in df.columns:
                ...
                expanding_range = expanding_max - expanding_min
                
                # Normalize using expanding window statistics
                # When range is 0 (constant value), set normalized value to 0.5 (midpoint)
                normalized = (df[col] - expanding_min) / expanding_range
                normalized = normalized.fillna(0.5)  # Fill NaN (when range=0) with 0.5
                
                df[f'minmax_{col}'] = normalized
