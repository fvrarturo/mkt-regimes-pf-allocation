"""
Regime-Conditional Analysis: Which Macro Variables Predict ERP Returns?

This analysis uses LAGGED macro variables to understand what PREDICTS returns
...
output_dir = Path(__file__).parent / 'results' / 'regime_predictors'
output_dir.mkdir(parents=True, exist_ok=True)

print("=" * 90)
print("REGIME-CONDITIONAL PREDICTORS OF ERP RETURNS")
print("=" * 90)
print()
print("Using LAGGED macro variables to predict ERP returns")
print("(Macro at t-1 predicts ERP at t)")
print()
print("Hypothesis: Different macro variables matter in different regimes")
print("  - Inflation might drive returns in high-inflation regimes")
print("  - Growth might drive returns in growth regimes")
...
# Exclude volatility variables (they're not predictive, just contemporaneous)
volatility_vars = ['vix', 'nasdaq_vol_indx', '3month_vol_index_sp500']
macro_cols = [v for v in macro_cols if v not in volatility_vars]

print(f"Analyzing {len(macro_cols)} macro variables (excluding volatility)")
print()
...
variable_categories = {
    ...
    'Financial Conditions': ['nat_fin_condition_indx', '10y_2y_spread']
}
...
for regime in sorted(data['regime'].unique()):
    ...
    for var in macro_cols:
        ...
        # Prepare data
        corr_data = regime_data[['erp', lag_var]].dropna()
        ...
        if weights is not None:
            ...
            corr, pvalue = stats.pearsonr(corr_data['erp'], corr_data[lag_var])
            n_eff = len(corr_data)
...
for regime, regime_data_dict in results_by_regime.items():
    ...
    for var in macro_cols:
        ...
        # Prepare data
        reg_data = regime_data[['erp', lag_var]].dropna()
        
        if len(reg_data) < 10:
            continue
        
        X = reg_data[[lag_var]].values
        y = reg_data['erp'].values
...
print("=" * 90)
print()
print("This analysis uses LAGGED variables to identify what PREDICTS returns")
